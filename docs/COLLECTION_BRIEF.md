...
- **accuratus.com/{재료}.html** — 세라믹·유리의 유전율·손실계수·체적저항률을 **조건까지 붙여
  한 표에** 싣는다. **전기군에만 써라.**
  **⚠️ 이 표의 `Shear Modulus`는 역산이다** — 같은 표의 E와 ν로 `E/(2(1+ν))`를 계산하면
  마지막 자리까지 일치한다(알루미나 96%: 300/(2×1.21) = 123.97 = 인쇄값 124).
  SiC·Si₃N₄ 페이지는 G·K를 아예 `—`로 비운다. **Prony군을 이걸로 닫지 마라.**
  없는 슬러그는 **404 + 959 B**를 준다(종전 "200으로 준다"에서 바뀌었다).
...
- **흑연 전단탄성률의 전제를 바로잡는다.** 열분해흑연의 C₄₄는 **Blakslee 1970이 인쇄한
  0.27(9) GPa**다. 흔히 인용되는 4.5~5 GPa는 **무결점 격자값**(Bosak IXS 5.0, Nicklow INS 4.6)
  이거나 DFT다. **전위가 많은 실제 PGS·Kaneka 시트에는 0.27이 맞는 값이다.**
- **표 머리글이 뒤바뀐 채 인쇄될 수 있다 — 저자 오식이지 추출 아티팩트가 아니다.**
  12차가 PET Prony 표(IJMPT 60(1) 18)에서 잡았고 네 가지로 확정했다 —
  ① Σ(왼쪽)=0.999168 ≤ 1 ② Σ(오른쪽)=1003 ③ 오른쪽만 10자리 단조증가
  ④ G(∞)=0.71 MPa가 PET 얽힘 플래토와 일치. **bbox x좌표까지 확인해 추출 문제가 아님을 갈랐다.**
  `Sci. Rep. 14:14634`은 **Table 4/5 캡션이 서로 바뀌어** 있는데 PMC XML과 출판사 PDF에
  동일하게 있어 **PDF 대조로는 안 잡힌다.**
- **⚠️ 사이트가 죽었다는 보고는 직접 확인하고 믿어라.** `makeitfrom.com`과 `matmake.com`이
  각각 배치 보고로 "전면 차단"으로 옮겨졌다가 **둘 다 살아 있음이 확인됐다**(makeitfrom의
  2,194 B는 404 본문, matmake는 200 + 14 KB). **간헐적 실패를 영구 차단으로 적으면
  다음 파동이 유효한 경로를 통째로 버린다.**
- **벤더 문서는 개정판마다 값이 다를 수 있다 — 오식이 아니다.**
  DuPont Kapton 열전도율이 구판 `0.12 / ASTM F-433(1987)`, 현행 `0.20 / ASTM D5470`이다.
  **두 판 모두 자기 cal 환산이 정합**하므로(2.87e-4 및 4.8e-4 ×418.4) 어느 쪽도 오식이 아니라
  **시험법이 바뀐 것**이다. 카탈로그에 두 값이 섞여 있었다.
  **값이 다르면 오식부터 의심하지 말고 시험법·개정일을 먼저 비교하라.**
- **`Thermal Conductance [kW/m²·K]`를 열전도율로 읽지 마라.** 단위가 다르다(두께로 나눈 양이다).
  Heraeus 본딩와이어 브로슈어의 Au 31.1 / Cu 39.4가 그 함정이고, 정작 열전도율은
  별도 팩트시트에 401 W/(m·K)로 있다(`* Based on Core Material` 각주 확인 필요).
- **같은 표의 두 행이 서로 모순될 수 있다 — SI 행을 써라.** Piezo.com PZT 표가
  `W/cm·°C 1.9~2.3`(= 190~230 W/m·K, PZT에 불가능)과 `W/m·K 1.2`를 인접 행에 인쇄한다.
- **벤더가 PDF에는 안 싣고 제품 페이지에만 싣는 경우가 있다.** Panasonic MEGTRON은
  R-5775/R-5670 PDF를 grep하면 `conductivity` 0건이라 "발표 안 함"으로 오판하기 쉬운데,
  `na.industrial.panasonic.com/.../model/{id}` 사양표가 값을 준다.
  **⚠ 그 셀의 단위 표기가 `W/m · cm`로 오식돼 있으니 행 머리글을 읽어라.**
- **검색 요약이 없는 숫자를 창작한다 — 원문에서 문자열로 확인하라.**
  12차가 두 건을 잡았다 — "DELO KATIOBOND 4552 = 0.27 W/mK"는 DELO 문서 어디에도 없고,
  리튬 전해질에서는 **이온전도도 10.4 mS/cm를 열전도율로 혼동**해 흘린다.
- **arXiv DOI(`10.48550/arXiv.NNNN`)는 DataCite에 등록된다.** Crossref로 조회하면 404가
  나오지만 유효한 DOI다 — `api.datacite.org/dois/{doi}`로 확인하라.
- **"보강 근거"로 든 문장도 검증하라 — 값이 아니라 보강이 틀릴 수 있다.**
  12차에서 43행이 자기 값은 정직하게 `[가정] 측정값이 아니다`라고 밝히면서,
  뒷받침으로 **"3M VHB 4910 TDS 실측 0.49와도 일치"**라고 적고 있었다. **두 군데가 틀렸다** —
  ① 4910 개별 TDS가 아니라 **계열 기술자료(2005년판)**이고
  ② 3M이 시험법·변형률범위·온도를 하나도 안 적은 **모델링 상수**로 인쇄하므로 **실측이 아니다.**
  같은 박스가 `Youngs Modulus will be about 3 times the Shear Modulus`(= E=3G, 즉 ν=0.5)라고
  적어 **비압축 진술**임을 스스로 밝힌다. **값 자체는 유효했고 보강 문장만 거짓이었다.**
- **벤더가 현행판에서 뺀 값이 구판에는 있다.** 3M 현행 VHB 문서 4종에 `Poisson`이 0건인데
  **2005년판 "Typical VHB Tape Properties for Modeling" 박스에는 있다.**
  3M 자사 호스트에는 남아 있지 않고 **유통사 미러**(Digi-Key)로만 받힌다.
  **"현행 문서에 없다"를 "발표한 적 없다"로 읽지 마라.**
- **포아송비의 계열 전파에는 경계가 있다.** 12차가 확인한 것 —
  **PE 폼은 밀도가 아니라 셀 크기가 지배한다**(같은 논문에서 29 kg/m³·2 mm 셀은 **0.03**,
  37 kg/m³·0.1 mm 셀은 **0.37**). **VHB의 0.49를 폐쇄셀 PE 폼 테이프에 물려주면 안 된다** —
  VHB는 준비압축성 솔리드 아크릴이고 PE 폼은 실제로 압축성이다.
  **엘라스토머 0.49도 학계가 갈린다** — 구 실측군 0.46~0.49 대 신 측정군 0.4985~0.4995.
  **체적탄성률이 최대 10배 갈리므로** 한쪽을 단정하지 마라.
- **2차 "모델 파라미터 표"는 전 행이 검증에 실패할 수 있다 — 인용 출처를 실제로 열어라.**
  12차가 `Sensors 25(15) 4583` Table 4를 통째로 기각했다. 세 가지로 무너졌다 —
  ① Pyrex 행의 1차 출처(Bull. Mater. Sci. 18, 857)를 받아 전문을 읽었는데 **인쇄된 두 숫자가
  하나도 등장하지 않는다**(있는 것은 균열성장 지수 n=14.01과 불활성강도 270.3 MPa뿐).
  ② Silicon 행의 1차 출처 **저자가 인쇄된 이름과 다르다**(Crossref·Semantic Scholar 대조).
  ③ **지배식이 `N = C·(ΔS)^m`으로 뒤집혀** 인쇄돼 있다.
  **`api.semanticscholar.org/graph/v1/paper/DOI:{doi}?fields=authors` + Crossref 조합이
  오염된 참고문헌을 잡는 가장 빠른 수단이다.**
- **"코팅 피로" 논문의 S-N은 대개 기재(강·Ti)의 것이다.** 코팅 행에 옮기면 범주 오류다.
- **무차원 S-N 법칙은 정당한 대입 수단이다 — 단 정규화 분모를 확인하라.**
  `log(σa/σt0) = A + B·log Nf` 형태는 각 재료 자신의 강도를 곱해 쓰라고 저자가 의도한 것이다.
  **다만 같은 표 안에서도 행마다 분모가 다를 수 있다**(IG-11 표에서 Ring 행은 링압축강도,
  Uniform 행은 인장강도로 나눈다).
- **유리의 사이클 기준 S-N은 물리적으로 존재하지 않는다.** 붕규산유리의 순환 데이터가
  **등가파단시간 축에서 정적피로 띠에 겹치기** 때문에 이 분야는 사이클 S-N을 만들지 않고
  **정적피로(σⁿ·t = B)와 동적피로(응력속도)**만 발표한다(材料 40(454) 914 / 40(458) 1491).
...
- **대조군이 본체와 같은 값을 주면, 그 값은 본체의 성질이 아니다.**
  ...
  같은 대역**이었다. **PSA를 PSA로 만드는 성분을 빼도 숫자가 안 변하면, 그 숫자는
  CaCO₃ 37 wt% 충전 매트릭스가 지배하는 값이다.** 무충전 제품에 옮길 수 없다.
  **표에 대조군 행이 있으면 반드시 함께 읽어라 — 그 행이 값의 정체를 알려준다.**
- **표의 한 열만 틀렸을 수 있다 — 다른 열로 쌍의 정합을 확인하라.**
  같은 파동에서 `Polymers 2026, 18(4), 538` Table 2의 `E 2.371 MPa ↔ UTS 0.096 MPa`가
  어긋나 보였는데, **Toughness 열로 교차검산하니 평균응력/UTS 비가 세 행 모두 0.56~0.66**으로
  일치했다. **UTS·파단연신율 쌍은 정합하고 E 열 하나만 튄 것**이다(E/UTS가 PDEAE 2.2인데
  PDA만 24.7). 그래서 그 쌍은 쓰고 **탄성률은 한 칸도 넣지 않았다.**
  **표 전체를 버리기 전에 어느 열이 문제인지 좁혀라.**
