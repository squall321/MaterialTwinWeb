...
- **matmake.com은 재료군마다, 그리고 물성 열마다 신뢰도가 다르다.**
  ...
  - **사이트 단위로 신뢰/불신을 판정하지 마라**(AZoM도 같다 — PU 선팽창은 정상인데
    E-SBR은 체적팽창계수로 보인다).
- **"벤더가 발표하지 않는다"와 "문헌에도 없다"는 다른 명제다.**
  카탈로그가 "PCB·RF 라미네이트는 피로를 어느 벤더도 발표하지 않는다"를 부재 사유로 적어
  왔는데, **전제는 참이고 결론이 틀렸다.** 10차가 학술 문헌에서 직조유리/에폭시 적층판의
  S-N 회귀식을 찾아 19종을 닫았다(IEC 60893 EPGC 203 축하중 `S=417.29·N^-0.106`,
  JIS-K6912 굽힘 `S=740·N^(-1/7.5)` — 둘 다 **그림 범례에 문자로 인쇄된 회귀식**이라
  곡선 판독이 아니다). **벤더 문서가 없다고 포기하기 전에 학술 문헌을 따로 훑어라.**
...
- **표의 N이 사이클이 아니라 킬로사이클일 수 있다 — 본문 서술을 믿지 마라.**
  NIST Monograph 177의 베릴륨동 회귀식은 본문이 N을 "the number of cycles to failure"라고
  쓰는데, **회귀는 표 머리글이 `Cycles, N (10³)`인 표 값 그대로** 돌린 것이다.
  그대로 쓰면 **수명이 1000배 짧게** 나온다. 11차가 세 가지로 확정했다 —
  ① 595개 점의 잔차가 계통적으로 정확히 `−log₁₀(1000)`만큼 어긋난다
  ② 590 MPa에서 188 사이클이 나오는데 UTS 1250 MPa 재료로는 불가능하다
  ③ 독립 논문의 1e6 사이클 값과 보정 후 21% 이내로 맞는다(보정 전은 2배 이상 어긋난다).
  **회귀식을 옮기기 전에 그 회귀가 어느 표를 대상으로 돌아간 것인지 확인하라.**
- **점근응력이 음수로 인쇄될 수 있고, 그때 원문이 스스로 못 믿겠다고 밝힌다.**
  NIST 식 (11-9)의 σ₀ = −4.03 MPa인데 같은 논문이 *"the uncertainty of the last constant
  exceeds the value of the constant"*(표준편차 114.50 대 값 4.03)라고 적는다.
  `conditions.asymptote_not_determined: true`를 달고 **그 값을 피로한도로 읽지 마라.**
- **"순환소성이 없다"에서 "피로 데이터가 없다"로 넘어가지 마라.**
  9차가 "세라믹은 피로계수가 없다"고 선언했으나 **틀렸다** — 11차가 알루미나의 S-N 회귀식을
  찾았다(Maekawa 1988, 材料 37(415) 441). **응력-수명(Basquin/S-N)은 세라믹에도 존재한다.**
  없는 것은 소성을 전제한 정식(Coffin-Manson·Morrow·Darveaux)뿐이다.
  **부재를 선언할 때 "어느 정식이 없는가"까지 좁혀서 적어라.**
- **피로계수는 규약이 두 가지다. 섞으면 소성계수가 52% 어긋난다.**
  **Manson 1965만 `Δε` 대 `Nf`(전변형률 범위·사이클)**이고 **나머지 추정식은 전부
  `Δε/2` 대 `2Nf`(진폭·반전수)**다. 섞으면 탄성계수는 2^0.12(약 8.7%),
  소성계수는 2^0.6(**약 52%**) 어긋난다. **어느 규약인지 notes에 반드시 적어라.**
- **디렉터리 순회는 지연을 넣어라 — 사이트를 통째로 잃는다.**
  9차 피로 배치가 `fde.uwaterloo.ca`를 무지연으로 610쪽 받은 뒤 **IP 차단**을 맞았고
...
- **Cowper-Symonds의 C를 크기만 보고 판단하지 마라 — DIF로 환산해서 봐라.**
  10차가 `Al6063-T6`의 `C = 9.39e10 1/s`를 "자릿수가 비정상"이라고 이상값으로 보고했으나,
  우리 규약 `1+(ε̇/C)^(1/p)`에 p=9.55를 넣으면 **1000/s에서 DIF 1.15**로 알루미늄답게
  율속 둔감하다. **정상값이다.** 오히려 같은 카탈로그에서 `6500/4`를 쓰는 다른 Al들이
  1000/s에서 1.63을 준다. **C와 p는 짝으로만 의미가 있다** — 한쪽만 보면 반드시 오판한다.
  점검 방법은 이 한 줄이다.
  ```
  DIF(edot) = 1 + (edot/C)**(1/p)   # 1 / 100 / 1000 s^-1 에서 찍어 봐라
  ```
