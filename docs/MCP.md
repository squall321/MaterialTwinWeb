...
LLM(Claude 등)이 MaterialTwin 물성 DB를 자연어로 **조회·등록·수정·삭제**하도록 노출하는 MCP(stdio) 서버. `.mcp.json`으로 연결되며 웹 API와 동일한 DB·계산 경로를 공유한다.

- **엔트리**: `backend/mcp_server.py` (FastMCP `"materialtwin"`)
- **연결**: `.mcp.json` → `backend/.venv/bin/python backend/mcp_server.py`
- **데이터**: 웹과 동일한 SQLite(`var/data/materialtwin.db`) + 곡선 Parquet
- **노출**: 도구 20 · 리소스 2 · 프롬프트 2 (프로토콜 왕복 검증 완료)
...
| `get_mat_card(test_id, units, model)` | LS-DYNA 카드 텍스트(*MAT_024·*MAT_098·*MAT_VISCOELASTIC) |
...
| 도구 | 역할 |
|---|---|
| `register_material(name, category, material_code, description)` | 새 재료 등록 |
| `register_tensile_test(material_id, strain[], stress_mpa[], …)` | 인장곡선 등록 → 시편 자동생성·물성·구성방정식 4모델 피팅까지 |
| `register_relaxation_test(material_id, G0/Ginf/beta 또는 time_s/modulus_mpa, …)` | 점탄성 완화 등록(Prony 파라미터 또는 실측 곡선) |
| `update_material(material_id, …)` | 재료 메타데이터 부분 수정 |
| `delete_material(material_id, confirm)` | 재료+하위 삭제(confirm=False면 미리보기) |
| `delete_test(test_id, confirm)` | 시험 1건 삭제(confirm 2단계) |
