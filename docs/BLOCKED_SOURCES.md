...
## AA. 링크 부패를 처음 측정했다 (2026-08-10)

10차 항복 배치가 "DB의 rogerscorp URL이 500자 제한으로 잘려 있다"고 보고했다.
**확인 결과 잘린 URL은 0건이다**(480자 이상도 0건). 진단은 틀렸다.
**그러나 결함은 실재했다** — `ultralam-3850ht-data-sheet.pdf`가 404 + 110 KB HTML을 준다.
**잘린 게 아니라 벤더가 파일을 내린 것**이다.

그래서 벤더·웹 출처 1,182건에서 90건을 무작위 표본으로 검사했다.

| 결과 | 건수 | 성격 |
|---|---:|---|
| 정상 | 81 | |
| **403 / 406** | **5** | **우리가 차단된 것 — 문서는 살아 있다**(ScienceDirect, ULProspector, tangram, astmsteel) |
| **404** | **2** | 실제 소멸 (lohmann DuploCOLL 56003, ITEQ IT-968) |
| **연결 실패(000)** | **2** | nitto.com, circuitfoil.com — 간헐적일 수 있다 |

**실제 소멸률은 약 2~4%다.** 값 자체는 수집 시점에 원문 대조로 검증했으므로 무효가 되지
않는다. 위험은 **나중에 재검증이 불가능해지는 것**이다.
