...
def main():
    ...
    _tbl(doc, ["지표", "정의", "무엇을 재는가"], [
        ...
        ["실측기반 채움률", "tier4(가정·계산)를 뺀 칸 / 전체 칸", "근거의 두께"],
        ["재료 준비율", "그 해석을 실제로 돌릴 수 있는 재료 / 대상 재료", "해석 가능성"],
    ])
    doc.add_paragraph()
    # 두 지표가 가장 크게 갈리는 해석을 매번 찾아서 예로 든다(손으로 적으면 곧 어긋난다).
    _split = max(cov, key=lambda x: x["cell_pct"] - x["ready_pct"])
    _p(doc, "셀 채움률이 높아도 재료 준비율은 낮을 수 있다. 재료마다 비어 있는 칸이 다르면 "
            f"그렇게 된다 — {_split['name']}이 정확히 그 경우다"
            f"(셀 {_split['cell_pct']:.1f}% vs 준비 {_split['ready_pct']:.1f}%). "
    ...
    for x in cov:
        ...
        rows.append([x["name"], x["n_target"],
                     f"{x['filled']}/{x['cells']}",
                     f"{x['cell_pct']:.1f}%  {bar(x['cell_pct'], 10)}",
                     f"{x['meas_pct']:.1f}%",
                     f"{x['n_ready']}  ({x['ready_pct']:.1f}%)",
                     f"{label[:36]} — {top[1]}종"])
    _tbl(doc, ["해석", "대상", "셀", "셀 채움률", "실측기반", "재료 준비", "가장 큰 공백"],
         rows, bsz=7.5)
    doc.add_paragraph()
    # 가정값이 만든 착시를 문서에서도 나란히 보인다 — 지표 하나만 실으면 과대평가된다.
    _gap = sum(x["cells"] for x in cov) and (
        sum(x["filled"] for x in cov) - sum(round(x["meas_pct"] * x["cells"] / 100) for x in cov))
    _p(doc, f"**실측기반**은 tier4(가정·계산)를 뺀 값이다. 두 열의 차이 약 {_gap:,}칸이 "
            "가정값으로 채워진 부분이다. 해석을 돌리기 위해 칸을 닫는 것과, 그 값이 실측에 "
            "근거하는 것은 다른 문제다 — 둘을 나란히 둔다.", bold=True)

    _p(doc, "읽는 법", bold=True, space=2)
    # 구간을 손으로 적으면 파동마다 위 표와 어긋난다. 매번 실제 값에서 만든다.
    def _band(lo, hi):
        xs = sorted((x for x in cov if lo <= x["ready_pct"] < hi),
                    key=lambda x: -x["ready_pct"])
        return " · ".join(f"{x['name']} {x['ready_pct']:.1f}%" for x in xs) or "해당 없음"
    _tbl(doc, ["구간", "해석", "의미"], [
        ["준비율 50% 이상", _band(50, 1e9), "기본 물성만 쓰는 해석. 지금 바로 돌아간다"],
        ["준비율 25~50%", _band(25, 50), "부품군을 좁히면 실용 가능"],
        ["준비율 10~25%", _band(10, 25), "그 해석의 핵심 물성이 아직 절반도 안 찼다"],
        ["준비율 10% 미만", _band(0, 10), "그 해석을 그 해석답게 만드는 물성이 비어 있다"],
    ])
    doc.add_paragraph()
    _low = [x for x in cov if x["ready_pct"] < 10]
    _p(doc, f"낮은 쪽 {len(_low)}개가 이 카탈로그의 실제 병목이다. 기본 물성"
            "(밀도·영률·포아송·k·cp·CTE)은 대부분 채워졌는데, 율속·점탄성·젖음성·피로계수·"
            "광물리처럼 그 해석의 정체성을 이루는 물성은 여전히 낮다.", bold=True)
    ...
    for i, x in enumerate(cov, start=1):
        ...
        _tbl(doc, ["대상 재료", "셀 채움", "실측기반 채움", "재료 준비", "실측기반 준비"],
             [[f"{x['n_target']}종", f"{x['filled']}/{x['cells']}  ({x['cell_pct']:.1f}%)",
               f"{x['meas_pct']:.1f}%",
               f"{x['n_ready']}종  ({x['ready_pct']:.1f}%)",
               f"{x['n_ready_meas']}종  ({x['ready_meas_pct']:.1f}%)"]])
        if x["missing"]:
            doc.add_paragraph()
    ...
    _p(doc, "재료 준비율을 가장 크게 올리는 순서다. 준비율은 이번 판 실제 값이다.")
    _rp = {x["name"]: x for x in cov}

    def _r(name):
        x = _rp.get(name)
        return f"{name} 준비율 {x['ready_pct']:.1f}%" if x else name
    _tbl(doc, ["순위", "물성", "막고 있는 것", "성격"], [
        ["1", "율속(Cowper-Symonds · DIF · LCSR · JC-C)",
         _r("낙하·충격"), "수집 여지 있음. 구리·황동은 원출처 미색인"],
        ["2", "비열 · 열전도율",
         f"{_r('열전달·방열')} · 결로 두 갈래를 동시에 막는다",
         "CCL·자성재·커버글라스는 벤더가 아예 안 싣는다"],
        ["3", "점탄성 Prony 세트",
         _r("폴더블 벤딩"), "OA에 OCA·무색 PI가 없다. 유료 접근 또는 자체 DMA"],
        ["4", "젖음성(접촉각 · 표면에너지)",
         _r("결로(표면)"), "커버글라스는 문헌 자체가 없다. 세정 이력 명시본만 유효"],
        ["5", "피로계수(Basquin · Coffin-Manson · Darveaux · Morrow)",
         _r("피로·수명"), "솔더·구조금속에 집중하면 소수로 큰 효과"],
        ["6", "광물리(τ₀ · k_q) · 산소 수송(D · S)",
         f"{_r('산소소광-발광체')} · {_r('산소확산-매트릭스')}",
         "적색발광 과제. OLED 적층 산소확산 모델은 PDE/oxygen-diffusion 참조"],
    ], bsz=7.5)
    doc.add_paragraph()
