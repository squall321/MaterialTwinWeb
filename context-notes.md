...
## 2026-07-16 — HEAXHub 포털 재배포 (포트 충돌 복구 + 실데이터 시드)

**증상**: 배포 앱이 포트 9100을 배정받았는데 그건 Prometheus node_exporter 소유 → 앱이 사실상 접근 불가(curl 200은 node_exporter 응답이었음).
**근본원인**: HEAXHub port_allocator가 DB 추적만 하고 OS 레벨 점유를 확인 안 해 9100(외부 리스너)을 그대로 배정.
**수정(HEAXHub, 커밋 59c666e)**: allocator에 socket bind 검사 추가 — 신규/재사용 배정 모두에서 실제 점유 포트를 건너뜀. _port_is_free(9100)=False 검증.
**부수 발견**: 배포 볼륨 DB가 구 스키마(create_all 출신, alembic 미적용 — relaxation CHECK/AUTOINCREMENT/UNIQUE 없음)라 점탄성 등록 실패. init_db=create_all은 기존 테이블을 ALTER 안 함 → 볼륨 지속 시 스키마 드리프트. (durable 후속: 부팅 시 alembic upgrade. 이번엔 볼륨 DB를 새 스키마로 재생성.)
**재기동(백엔드 재시작 없이)**: 고친 launcher 코드로 stop("materialtwin_web")→포트 해제·9100 할당삭제→볼륨 DB 재생성(부팅 create_all=현재 스키마)→launch()→**포트 9118 배정(9100 스킵)**+caddy 등록. 다른 앱은 독립 인스턴스라 무영향.
**시드**: drive-sync sync-from-drive(HEAX_DATA_DIR=배포볼륨)로 Drive 번들 병합 → 70종(sha256 OK).
**검증**: Caddy 포털(:4180/apps/materialtwin_web/) 브라우저 — 라이브러리 70건(폴리머·고무·폼 포함), 인사이트(Ashby·박스플롯·히스토그램·지식그래프·커버리지), 17-4PH 상세 곡선(Rm/Rp0.2 마커)까지 전부 렌더. 배포 최종: port 9118, caddy_registered True.
**후속(비차단)**: 앱 부팅을 create_all→alembic upgrade head로 바꿔 볼륨 스키마 드리프트 근절.

## 2026-07-16 — 후속: 부팅 스키마 마이그레이션 (볼륨 드리프트 근절)

app/db.py init_db를 create_all → alembic 관리로 교체(_apply_schema):
- 빈 DB: create_all + `alembic stamp head`(현재 모델로 생성, 버전화 — 테스트 빠름).
- 버전 DB: `alembic upgrade head`(누적 마이그레이션 적용 — 구 배포 볼륨 자동 최신화).
- 레거시(테이블 有·버전 無): create_all 보완 + stamp head + 경고.
이로써 볼륨이 지속돼도 스키마가 head로 수렴 — create_all이 기존 테이블을 ALTER 안 해 생기던 배포 볼륨 드리프트(=이번 포털 점탄성 실패 원인) 근절.
검증: pytest 118 passed. 신규 회귀 2건 — test_boot_migrates_old_versioned_db(c7b6cca38dc2 구스키마→부팅→head, relaxation CHECK·AUTOINCREMENT·UNIQUE 확인), test_boot_fresh_db_is_versioned_at_head. init_db는 모듈 레벨 engine 의존이라 서브프로세스로 테스트(리로드 오염 회피). dev DB·배포 볼륨 모두 head로 stamp 완료, 배포 서비스 70종 정상.

## 2026-08-19 — 43차 EE: 키 열 개를 열다 (채굴이 아니라 자물쇠)

**전제** — 43차 EB(카메라)·ED(배터리·음향)가 값을 확보하고도 카탈로그에 키가 없어 버렸다.
이 배치는 새로 캐지 않고 **그 키를 열어 지목된 값만 넣는다.**

### 키를 가른 기준 — 브리프 453(차원이 정한다)의 세 가지 얼굴

1. **차원이 다르면 무조건 가른다.** Chen 2021 이 `peel strength` 라 부르는 값의 정의가
   `σ = Fmax/S` 라 **Pa** 다. 우리 `interface.peel_strength` 는 **N/m** —
   이름을 믿었으면 1.41 MPa 가 1.41 N/m 로 들어가 여섯 자릿수가 틀어졌다. → `interface.tensile_adhesion_strength`
2. **차원이 같아도 측정량이 다르면 가른다.** 이 taxonomy 는 이미 그렇게 되어 있다 —
   `cure_shrinkage`·`mold_shrinkage`·`compression_set`·`elongation_at_break` 가 전부 무차원 변형률인데 키가 넷이다.
   경도는 **면적 규약**으로 갈려 있다(Meyer 투영 · Martens 접촉표면 · H_IT 투영접촉).
   ISO 2039-1 은 구면 자국의 **곡면적**이라 넷째다. Ts(새그온도)도 Tg 와 같은 K 지만
   HOYA 가 **같은 행에 둘 다 인쇄**하므로 한 키에 넣으면 그 쌍이 깨진다.
3. **차원도 측정량도 같고 시험 파라미터만 다르면 조건축으로 올린다.** 걸리 투기도의 `100 cm³`·
   압력·면적은 조건축이다 — `s/100cm³` 로 굳히면 50 cc 판·ASTM D726 판이 들어올 자리가 없다(로크웰 선례).

### 단위 규약 — 기존 키가 정한다

- **(BH)max = J/m³.** 기존 자기 키가 전부 SI 기본단위(remanence T · coercivity A/m)라 맞췄다.
  시트의 kJ/m³ 는 접두어만 벗겼다. **MGOe 는 환산해 넣지 않았다** — 같은 표에 kJ/m³ 열이
  있으면 그쪽이 정본이고(§136), MGOe 열은 **검산기로만 썼다**(§225, ±1% 이내).
- **관통강도 = N.** gf 는 힘 단위이고 1 gf = 9.80665e-3 N 은 CGPM 1901 표준중력에서 나오는
  **정의값**이라 가정이 없다. 응력으로는 못 간다 — 바늘 끝 면적을 시트가 안 찍는다(§117).
- **광탄성계수 = 1/Pa.** 1 cm²/dyne = 1e-4 m² / 1e-5 N = **10 Pa⁻¹**(정의 환산).
  1 Brewster = 1e-12 Pa⁻¹ 이므로 ZEONEX 480R 6.5 · PC 72 · PMMA 6.0 Brewster 로 읽힌다.

### 물리 상한 — 검사기에 상시로 걸었다

`(BH)max ≤ Br²/(4µ₀)`. NdFeB 8등급 × min/typ 16칸 + 페라이트 7칸이 전부 통과했고
**비가 0.90~0.97 에 모여 있다** — 실제 소결자석이 사각형 상한의 95% 언저리에 오는 것과 맞다.
`integrity_check.bad_energy_product()` 가 재료의 최대 Br 로 상한을 잡아 상시 감시한다.

**두 가지 함정을 여기서 배웠다.**
- **짝이 되는 칸이 없으면 없는 위반이 만들어진다.** 페라이트 구간표는 Br 도 BHmax 도 구간인데
  앞선 파동이 Br 하한만 넣어 뒀다. BHmax **상단**을 Br **하단**으로 재면 위반이 뜬다 —
  그래서 Br 상단 두 칸(0.40 · 0.45 T)을 같이 넣었다(§137 의 확장, §76 반쪽 구간도 같이 닫혔다).
- **판정은 인쇄 반올림 구간으로 한다**(§339). Y30H-1 상단 32.0 이 31.83 을 0.5% 넘는데,
  Br `400 mT` 는 [395,405) 이라 상한이 31.04~32.63 구간이다. 검사기에 2% 여유를 뒀다 —
  진짜 단위 오입력은 ×1000·×7.96 이라 2% 와 섞이지 않는다.

### 로더 결함 — **브리프를 지킨 배치가 벌을 받고 있었다**

451(구간 중앙값은 계산값이다)을 읽은 배치는 근거에 *"중앙값은 만들지 않았다"* 라고 적는다.
적재기의 강등 규칙이 글자만 봐서 **그 행들을 `measured` → `computed` 로 내렸다.**
EE 자기 행 7개에서 발견했고, ED-3 의 인쇄된 Br 상단 0.40 T 도 같은 이유로 computed 였다.

- `asserts_midpoint(notes)` — 부정어 창을 **앞뒤로** 본다(한국어는 뒤 `않았다`, 영어는 앞 `not`).
- `integrity_check` 의 같은 항목을 SQL → 파이썬으로 옮겨 **같은 판정**을 쓰게 했다.
  두 스크립트가 standalone 이라 여섯 줄이 나란히 있다 — 한쪽만 고치면 갈린다.
- 옮기니 옛 SQL 이 놓치던 1건이 드러났다(영어 `midpoint of` 만 있어 한글 LIKE 에 안 걸렸다).
- **강등 시 `method_before_correction` 을 남기게 했다.** 표시가 없어 기존 441행은
  "가드가 내린 것"과 "배치가 그렇게 적어 보낸 것"을 구별할 수 없다 — 되돌리지 못하고 인계한다.

### 마이그레이션 — 예행 4종

`f7a8b9c0d1e2`. **전제를 두지 않는다**(b2c3d4e5f6a7 이 겪은 함정) — 키가 없으면 넣고 있으면 지나간다.
① 운영 head 사본(e5f6a7b8c9d0, 246정의) → 10/10 삽입 ② 구버전 사본(9d623ccf6364, 243정의) →
중간 5개 + 내 것까지 전부 적용, 10/10 ③ 이미 키가 있는 사본에 재적용 → 중복 0(멱등)
④ downgrade — 값이 붙은 정의는 남기고(256 유지) 빈 정의만 되돌린다(246).
